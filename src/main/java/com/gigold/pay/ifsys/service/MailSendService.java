...
public class MailSendService extends AbstractService {
    ...
    public void sendWithTemplateForHTML(Map<String,Object> model){
        ...
        try {
            messageHelper.setTo(getAddressTo());
            messageHelper.setFrom(simpleMailMessage.getFrom());
//            messageHelper.setFrom("chenkuan");
            messageHelper.setSubject(this.getSubject());
            String result = null;
            try {
                result = VelocityEngineUtils.mergeTemplateIntoString(this.getVelocityEngine(), this.getTemplateName(),
     ...
     * 利用模版发送HTML邮件,带发件人
     */
    public void sendWithTemplateForHTML(Map<String,Object> model,String sendFrom){
        mailSender = this.getMailSender();
        MimeMessage mimeMessage = mailSender.createMimeMessage();
        MimeMessageHelper messageHelper = new MimeMessageHelper(mimeMessage);
        try {
            messageHelper.setTo(getAddressTo());
            messageHelper.setFrom(simpleMailMessage.getFrom(),sendFrom);
            messageHelper.setSubject(this.getSubject());
            String result = null;
            try {
                result = VelocityEngineUtils.mergeTemplateIntoString(this.getVelocityEngine(), this.getTemplateName(),
