 ...
 * 拦截所有的html请求
 */
public class SessionFilter extends AbstractService implements Filter {
	/**
	 * 
	 */
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (flag) {
			...
			if (session.getAttribute(Constant.LOGIN_KEY) == null) {
				httpResponse.sendRedirect(httpRequest.getContextPath() + "/new/index.html?ref="+requestUrl);
				return;
			} else {
				chain.doFilter(request, response);
			}
